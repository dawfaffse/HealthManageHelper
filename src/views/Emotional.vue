  ...
  <div class="emotional-container">
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column  label="生活指标" width="120" >
        <template #default="scope">
           <div>
            <p>睡眠：{{scope.row.sleepQuality||3}}/5

            </p>
            <p>压力:{{scope.row.stressLevel||5}}/5</p>
           </div>
        </template>
      </el-table-column>
      ...
      <el-table-column  label="操作" width="240" fixed="right">
        <template #default="scope">
          ...
          <el-button text type="primary" size="default"  @click="handleDetail(scope.row)">详情</el-button>
    ...
    <div style="margin-top: 20px; display: flex; justify-content: flex-end;">
      ...
      <el-dialog v-model="detailDialogVisible" title="情感日志详情" width="800px" :close-on-click-modal="false">
  ...
  <div class="detail-section">
   ...
   <div class="detail-section">
    ...
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="主要情绪"><el-tag :type="getEmotionTagType(currentDetail.dominantEmotion)">{{currentDetail.dominantEmotion||'无'}}</el-tag></el-descriptions-item>
      <el-descriptions-item label="睡眠质量">{{currentDetail.sleepQuality||3}}/5</el-descriptions-item>
      <el-descriptions-item label="压力水平">{{currentDetail.stressLevel||5}}/5</el-descriptions-item>
   ...
   <div class="detail-section">
    ...
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="主要情绪">
        <el-tag :type="getAiEmotionTagType(aiData.primaryEmotion)">{{aiData.primaryEmotion||'无'}}</el-tag></el-descriptions-item>
      <el-descriptions-item label="情绪强度"><el-progress :percentage="aiData.emotionScore||50" :color = "getEmotionScoreColor(aiData.emotionScore)" :stroke-width="8" /></el-descriptions-item>
       <el-descriptions-item label="风险等级">
        <el-tag :type="getAiEmotionTagType(aiData.riskLevel)">{{aiData.riskLevel||'无'}}</el-tag></el-descriptions-item>
        <el-descriptions-item label="情绪性质">
        <el-tag :type="aiData.isNegative?'danger':'success'">{{aiData.isNegative?'负面情绪':'正面情绪'}}</el-tag></el-descriptions-item>
    </el-descriptions>
      ...
      <div class="ai-improvements-section">
        <h5>改善建议</h5>
           <ul class="improvement-list">
            <li v-for="(item,index) in aiData.improvementSuggestion" :key="index">{{item}}</li>
           </ul>
      </div>
...
<el-divider style="margin-top: 20px; font-size: 14px;"  border-style="dashed" content-position="left">创建时间: {{currentDetail.createdAt}}</el-divider>
...
const handleSearch = async (formData = {}) => {
  try {
    ...
    const res = await getEmotionalDiary(params)
    console.log('情感日志数据:', res)
    tableData.value = res.records || []
    pagination.value.total = res.total || 0
  } catch (error) {
    console.error('获取情感日志失败:', error)
  }
}
...
const handleDetail = (row) => {
  ...
  if(row.aiEmotion){
    console.log('AI情绪分析结果:', row.aiEmotion)
    aiData.value = JSON.parse(row.aiEmotion)
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    deleteEmotionalDiary(row.id).then(() => {
      handleSearch()
    })
  }).catch(() => {
    // 用户取消删除，不做任何操作

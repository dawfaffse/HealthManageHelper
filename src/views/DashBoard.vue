<template>
<div class="dashboard-container" v-loading="loading">
    <el-row :gutter="20">
        <el-col :span="6" :xs="24" :sm="12" :md="8" :lg="6" :xl="6">
           <el-card v-if="aiData.systemOverview">
            <div class="card-content">
                <div class="avatar users">
                    <el-image :src="iconUrl1" style="width: 40px;height: 40px;"></el-image>
                </div>
                <div class="infp">
                    <p class="title">总用户数</p>
                    <p class="number">{{ aiData.systemOverview.totalUsers}}</p>
                    <p class="subtitle-title">活跃用户:{{ aiData.systemOverview.activeUsers}}</p>
                   
                </div>
                
                
            </div>
           </el-card>
           </el-col>
           <el-col :span="6" :xs="24" sm="12" md="8" lg="6" xl="6">
           <el-card v-if="aiData.systemOverview">
            <div class="card-content">
                <div class="avatar like">
                    <el-image :src="iconUrl2" style="width: 40px;height: 40px;"></el-image>
                </div>
                <div class="infp">
                    <p class="title">情绪日志</p>
                    <p class="number">{{ aiData.systemOverview.totalDiaries }}</p>
                    <p class="subtitle-title">今日新增:{{ aiData.systemOverview.todayNewDiaries}}</p>
                   
                </div>
                
                
            </div>
           </el-card>
           </el-col>
           <el-col :span="6" :xs="24" :sm="12" :md="8" :lg="6" :xl="6">
          <el-card v-if="aiData.systemOverview">
            <div class="card-content">
                <div class="avatar comments">
                    <el-image :src="iconUrl3" style="width: 40px;height: 40px;"></el-image>
                </div>
                <div class="infp">
                    <p class="title">咨询会话</p>
                    <p class="number">{{ aiData.systemOverview.totalSessions }}</p>
                    <p class="subtitle-title">今日新增:{{ aiData.systemOverview.todayNewSessions}}</p>
                   
                </div>
                
                
            </div>
           </el-card>
           </el-col>
          
           <el-col :span="6" :xs="24" :sm="12" :md="8" :lg="6" :xl="6">
           <el-card v-if="aiData.systemOverview">
            <div class="card-content">
                <div class="avatar smile">
                    <el-image :src="iconUrl4" style="width: 40px;height: 40px;"></el-image>
                </div>
                <div class="info">
                    <p class="title">平均情绪</p>
                    <p class="number">{{ aiData.systemOverview.avgMoodScore }}/10</p>
                    <p class="subtitle-title">情绪健康指数</p>
                   
                </div>
                
                
            </div>
           </el-card>
          
         
        </el-col>
    </el-row>
    <el-row style="margin-top: 20px;" :gutter="20">
<el-col :span="12" :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
    <el-card v-if="aiData.systemOverview" style="width: 100%;">
      <template #header>
        <div class="card-header">
            情绪趋势分析
        </div>
      </template>
      <div class="card-content">
        <div ref="emotionChartRef" style="width: 100%;height: 300px;"></div>
      </div>
        
    </el-card>
</el-col>

<el-col :span="12" :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
    <el-card v-if="aiData.systemOverview" style="width: 100%;">
      <template #header>
        <div class="card-header">
            咨询会话统计
        </div>
      </template>
      <div class="chart-content">
        <div class="consultation-stats">
            <div class="stat-item">
        <div class="stat-label">总会话数</div>
        <div class="stat-value">{{ aiData.consultationStats.totalSessions }}</div>
        </div>

            <div class="stat-item">
       <div class="stat-label">平均时长</div>
        <div class="stat-value">{{ aiData.consultationStats.avgDurationMinutes}}</div>
        </div>
            <div class="stat-item">
        <div class="stat-label">活跃用户
        <div class="stat-value">{{ aiData.systemOverview.activeUsers }}</div>
        </div>
        </div>
    </div>
        <div ref="consultationChartRef" style="width: 100%;height: 230px;"></div>
     </div>
     
        
    </el-card>
</el-col>
    </el-row>
     <el-row style="margin-top: 20px;" :gutter="20">
<el-col :span="24" :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
    <el-card v-if="aiData.systemOverview" style="width: 100%;">
      <template #header>
        <div class="card-header">
            用户活跃度趋势
        </div>
      </template>
      <div class="card-content">
        <div ref="activeChartRef" style="width: 100%;height: 400px;"></div>
      </div>
        
    </el-card>
</el-col>
</el-row>

</div>
    

</template>

<script setup>
import { getOverview } from '@/api/admin'
import { ref, onMounted, onUnmounted, nextTick} from 'vue'

import * as echarts from 'echarts'




const iconUrl1 = new URL('@/assets/images/users.png', import.meta.url)
const iconUrl2 = new URL('@/assets/images/like.png', import.meta.url)
const iconUrl3 = new URL('@/assets/images/comments.png', import.meta.url)
const iconUrl4 = new URL('@/assets/images/smile.png', import.meta.url)




const aiData = ref({})
let emotionChart = null
const emotionChartRef = ref(null)


const loading = ref(true)


/**
 * 初始化图表
 */
const initCharts = () => {
    if(!emotionChartRef.value){
        return
    }
    // 先销毁旧的图表实例
    if(emotionChart){
        emotionChart.dispose()
    }
    
    // 创建图表实例
         emotionChart = echarts.init(emotionChartRef.value)
         //获取数据
         const TrendDate = aiData.value.emotionTrend || []
        //配置项
        const option = {
            title: {
                text: '情绪趋势分析',
                textStyle: {
                    fontSize: 16,
                    color: '#2d3436',
                    fontWeight: 600,
                     left: 'center',
                     top: 10
                },
               
            },
            //鼠标上去的提示框
            tooltip: {
            // 1. trigger: 'item'（默认）
            // 触发条件：鼠标碰到数据点 / 柱子 / 扇形才显示提示
            // 适合图表：饼图、雷达图、散点图、地图
            // 特点：只显示当前点的信息
            // 2. trigger: 'axis'（最常用）
            // 触发条件：鼠标滑到坐标轴区域就显示
            // 适合图表：折线图、柱状图、曲线图
            // 特点：同一坐标轴上的所有系列数据一起显示
            // 3. trigger: 'none'
            // 触发条件：不自动触发
            // 用途：完全自己用代码控制显示 / 隐藏
            // 一般不用
                trigger: 'axis',
               borderColor: '#fab1a0',
               borderWidth: 1,
               textColor: '#2d3436'
            },
            //图例
            legend: {
                data: ['平均情绪评分','记录数量'],
                top:40
            },
            //控制容器的位置和大小
            grid:{
                //设置图表距离容器的边距
                left:'3%',
                right:'4%',
                bottom:'3%',
                top:'80',
            },
            //x轴的设置
            xAxis: {
                //X 轴的类型 = 类目轴
                type: 'category',
                name: '日期',
                data: TrendDate.map(item => item.date),
                //设置 X 轴那条横线的样式
                axisLine:{
                    lineStyle: {
                        color: '#2d3436'
                    }

                }
            },
            //y轴的设置
            yAxis: [    {
                type: 'value',
                name: '情绪评分',
                position: 'left',

                //设置 Y 轴那条横线的样式
                axisLine:{
                    lineStyle: {
                        color: '#2d3436'
                    }

                }
            },{
                type: 'value',
                name: '记录数量',
                position: 'right',
                //设置 Y 轴那条横线的样式
                axisLine:{
                    lineStyle: {
                        color: '#2d3436'
                    }

                }
            }],
            //数据的设置,每个系列对应一个y轴
            series: [{
                name: '平均情绪评分',
                //折线图
                type: 'line',
                data: TrendDate.map(item => item.avgMoodScore),
                //平滑曲线
                smooth: true,
                yAxisIndex: 0,
               //设置线的样式
                lineStyle: {
                    width:3,
                    color: '#fab1a0'
                },
                //设置点的样式
                itemStyle: {
                    width:10,
                    height:10,
                    borderRadius:5,
                    color:'#fab1a0'
                }
            },{
                name: '记录数量',
                //折线图
                type: 'line',
                data: TrendDate.map(item => item.recordCount),
                 smooth: true,
                 yAxisIndex: 1,
               //设置线的样式
                lineStyle: {
                    width:3,
                    color: '#faebaf'
                },
                //设置点的样式
                itemStyle: {
                    width:10,
                    height:10,
                    borderRadius:5,
                    color:'#faebaf'
                }
            }]
        }
        //设置图表的配置项
        emotionChart.setOption(option)
    }
 
const consultationChartRef = ref(null)
let consultationChart = null

const initConsultationCharts = ()=>{
    if(!consultationChartRef.value){
        return
    }
    // 先销毁旧的图表实例
    if(consultationChart){
        consultationChart.dispose()
    }

    const dailyTrend = aiData.value.consultationStats.dailyTrend || []

    consultationChart = echarts.init(consultationChartRef.value)

    const option = {
      
  title: {
    text: '咨询活动统计',
    textStyle: {
      fontSize: 16,
      fontWeight: 600,
      color: '#2d3436'
    },
    left: 'center',
    top: 10
  },
  tooltip: {
    trigger: 'axis',
    backgroundColor: 'rgba(255, 255, 255, 0.95)',
    borderColor: '#fab1a0',
    borderWidth: 1,
    textStyle: {
      color: '#2d3436'
    }
  },
  legend: {
    data: ['会话数量', '参与用户数'],
    top: 40,
    textStyle: {
      color: '#636e72'
    }
  },
  grid: {
    left: '3%',
    right: '4%',
    bottom: '3%',
    top: 80,
    containLabel: true
  },
  xAxis: {
    type: 'category',
    data: dailyTrend.map(item => item.date),
    axisLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.3)'
      }
    },
    axisLabel: {
      color: '#636e72'
    }
  },
  yAxis: {
    type: 'value',
    axisLabel: {
      color: '#636e72'
    },
    axisLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.3)'
      }
    },
    splitLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.1)'
      }
    }
  },
  series: [
    {
      name: '会话数量',
      type: 'bar',
      data: dailyTrend.map(item => item.sessionCount),
      itemStyle: {
        color: {
          type: 'linear',
          x: 0,
          y: 0,
          x2: 0,
          y2: 1,
          colorStops: [
            { offset: 0, color: '#74b9ff' },
            { offset: 1, color: '#0984e3' }
          ]
        }
      },
      barWidth: '40%'
    },
    {
      name: '参与用户数',
      type: 'bar',
      data: dailyTrend.map(item => item.userCount),
      itemStyle: {
        color: {
          type: 'linear',
          x: 0,
          y: 0,
          x2: 0,
          y2: 1,
          colorStops: [
            { offset: 0, color: '#fdcb6e' },
            { offset: 1, color: '#f39c12' }
          ]
        }
      },
      barWidth: '40%'
    }
  ]
}
 //设置图表的配置项
     consultationChart.setOption(option)
    

    
}

const activeChartRef = ref(null)
let activeChart = null

const initActiveCharts = ()=>{
     if(!activeChartRef.value){
        return
     }
    // 先销毁旧的图表实例
    if(activeChart){
        activeChart.dispose()
    }

    const activityData = aiData.value.userActivity || []
    activeChart = echarts.init(activeChartRef.value)

    const option = {
  title: {
    text: '用户活跃度趋势',
    textStyle: {
      fontSize: 16,
      fontWeight: 600,
      color: '#2d3436'
    },
    left: 'center',
    top: 10
  },
  tooltip: {
    trigger: 'axis',
    backgroundColor: 'rgba(255, 255, 255, 0.95)',
    borderColor: '#fab1a0',
    borderWidth: 1,
    textStyle: {
      color: '#2d3436'
    }
  },
  legend: {
    data: ['活跃用户', '新增用户', '日记用户', '咨询用户'],
    top: 40,
    textStyle: {
      color: '#636e72'
    }
  },
  grid: {
    left: '3%',
    right: '4%',
    bottom: '3%',
    top: 80,
    containLabel: true
  },
  xAxis: {
    type: 'category',
    data: activityData.map(item => item.date),
    axisLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.3)'
      }
    },
    axisLabel: {
      color: '#636e72'
    }
  },
  yAxis: {
    type: 'value',
    axisLabel: {
      color: '#636e72'
    },
    axisLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.3)'
      }
    },
    splitLine: {
      lineStyle: {
        color: 'rgba(244, 162, 97, 0.1)'
      }
    }
  },
  series: [
    {
      name: '活跃用户',
      type: 'line',
      data: activityData.map(item => item.activeUsers),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#a29bfe'
      },
      itemStyle: {
        color: '#a29bfe'
      },
      areaStyle: {
        color: {
          type: 'linear',
          x: 0,
          y: 0,
          x2: 0,
          y2: 1,
          colorStops: [
            { offset: 0, color: 'rgba(162, 155, 254, 0.4)' },
            { offset: 1, color: 'rgba(162, 155, 254, 0.1)' }
          ]
        }
      }
    },
    {
      name: '新增用户',
      type: 'line',
      data: activityData.map(item => item.newUsers),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#fdcb6e'
      },
      itemStyle: {
        color: '#fdcb6e'
      }
    },
    {
      name: '日记用户',
      type: 'line',
      data: activityData.map(item => item.diaryUsers),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#00b894'
      },
      itemStyle: {
        color: '#00b894'
      }
    },
    {
      name: '咨询用户',
      type: 'line',
      data: activityData.map(item => item.consultationUsers),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#fab1a0'
      },
      itemStyle: {
        color: '#fab1a0'
      }
    }
  ]
}
 //设置图表的配置项
     activeChart.setOption(option)
    

    
}

onMounted(async () => {
  loading.value = true
  await getOverview().then((res) => {
    console.log(res)
    aiData.value = res
    //初始化图表
    nextTick(() => {
    initCharts()
    initConsultationCharts()
    initActiveCharts()
    loading.value = false
  })
  })
})
</script>

<style lang="scss">
.dashboard-container {
    height: 100%;
    overflow: auto;
    .card-content {
      display: flex;
      align-items: center;
      .avatar {
        margin-right: 12px;
        width: 60px;
        height: 60px;
        border-radius: 12px;
        display: flex;
        align-items: center;
        justify-content: center;
        &.users {
          background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        }
        &.like {
          background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);
        }
        &.comments {
          background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);
        }
        &.smile {
          background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);
        }
      }
      .info {
        .title {
          font-size: 14px;
          color: #7f8c8d;
          margin-bottom: 4px;
        }
        .value {
          font-size: 24px;
          font-weight: 700;
          color: #2c3e50;
          margin-bottom: 4px
        }
        .subtitle-title {
          font-size: 12px;
          color: #95a5a6;
        }
      }
    }
    .chart-content {
      padding: 20px;
      height: 300px;
      position: relative;

      canvas {
        width: 100% !important;
        height: 100% !important;
      }

      .consultation-stats {
        display: flex;
        justify-content: space-around;
        margin-bottom: 20px;

        .stat-item {
          text-align: center;

          .stat-label {
            font-size: 12px;
            color: #7f8c8d;
            margin-bottom: 4px;
          }

          .stat-value {
            font-size: 18px;
            font-weight: 600;
            color: #2c3e50;
          }
        }
      }
    }
  }
</style>
